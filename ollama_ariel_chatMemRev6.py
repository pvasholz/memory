...
# --- Memory-block wrapper (authority experiment) ------------------------------
# The preamble around injected memories is itself a frame that asserts (or
# hedges) the content's authority, and is therefore part of what "channel"
# means. Third CLI argument selects it:
#   python ollama_ariel_chatMemRev2.py gemma ariel_wrap_r1 neutral
# Default is 'hedged' — the wrapper used in ALL runs to date, preserved
# verbatim for continuity.
WRAPPERS = {
    "hedged": ("Relevant prior context. Use only if directly helpful; "
               "do not treat it as a verdict or as stronger evidence than "
               "the current user message:"),
    "neutral": "From memory:",
}
WRAPPER_MODE = sys.argv[3] if len(sys.argv) > 3 else "hedged"
if WRAPPER_MODE not in WRAPPERS:
    print(f"Unknown wrapper '{WRAPPER_MODE}' — use one of {list(WRAPPERS)}. Defaulting to hedged.")
    WRAPPER_MODE = "hedged"
...
# Retrieval policy (MMR / Rev6): Maximal Marginal Relevance (Carbonell &
# Goldstein 1998). Retrieval bypasses mem.search: the probe is embedded via
# Ollama, candidate vectors are pulled straight from Qdrant, and selection
# greedily balances relevance to the query against similarity to what is
# already selected, so near-duplicate memories crush each other's marginal
# score and can occupy at most ~one slot. This is the ONLY difference from
# Rev5 — wrapper handling, directive exclusion, limit, and return shape are
# identical, so a Rev5-vs-Rev6 paired run isolates the selection algorithm.
RETRIEVE_LIMIT = 5          # memories injected per turn (same as Rev5)
CANDIDATE_POOL = 15         # top-by-similarity pool MMR selects from (== Rev5 overfetch)
MMR_LAMBDA = 0.3            # 1.0 = pure relevance (== naive top-k); 0.0 = pure diversity.
                            # 0.5 is provably insufficient against verbatim duplicates
                            # collinear with the query; 0.3 rescues displaced constraints
                            # in the measured store geometry.
EMBED_MODEL = "nomic-embed-text"

def _embed(text):
    """Embed with the store's embedding model (new endpoint, legacy fallback)."""
    try:
...
def _load_candidates():
    """Scroll this user's non-directive memories (text + vector) from Qdrant.
    Read-only; robust across client versions and mem0 payload-shape drift.
    Directives are excluded here exactly as in Rev5 (their own verbatim lane)."""
...
def mmr_select(qv, cands, k, lam):
    """Greedy MMR over (text, vec) candidates. Returns [(text, sim, rank)]."""
...
def retrieve_memories(query):
    ...
    memory_block = WRAPPERS[WRAPPER_MODE] + "\n" + "\n".join(memory_texts)
...
# --- Raw-memory seeding (register experiment) --------------------------------
# Stores the exact string as an ORDINARY memory: infer=False bypasses the
# extractor (no paraphrase, no third-person recasting), but no directive
# metadata is attached, so it lives in the normal lane — retrieved by
# similarity, competing like any other memory, never auto-injected.
# This isolates REGISTER (imperative vs. described fact) from CHANNEL
# (retrieved context vs. standing rule). Seed with:  !rawmem <text>
RAWMEM_PREFIX = "!rawmem"

def store_rawmem(text):
    try:
        mem.add(
            [{"role": "user", "content": text}],
            user_id=USER_ID,
            infer=False,                      # exact string, no extraction
        )
        return True
    except Exception as e:
        print(f"\n[Raw memory storage error: {e}]")
        return False
# -----------------------------------------------------------------------------
...
def chat():
    ...
    print(f"Memory wrapper: {WRAPPER_MODE}")
    ...
    while True:
        ...
        if user.lower().startswith(RAWMEM_PREFIX):
            raw_text = user[len(RAWMEM_PREFIX):].strip()
            if not raw_text:
                print("\n[Usage: !rawmem <text to store verbatim as an ordinary memory>]\n")
                continue
            if store_rawmem(raw_text):
                print(f"\n[Raw memory stored verbatim: \"{raw_text}\"]\n")
            continue
